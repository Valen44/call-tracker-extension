class CallTrackerDashboard {
  ...
  setupEventListeners() {
    ...
    document.getElementById("editCalls").addEventListener("click", () => {
      this.editCalls();
    });
  }
  ...
  updatePeriodStats() {
    ...
    const totalEarnings = this.filteredCalls.reduce(
      (sum, call) => sum + (call.earnings || 0),
      0
    );
    const totalDuration = this.filteredCalls.reduce(
    ...
    document.getElementById("periodTime").textContent =
  ...
  updateCallsTable() {
    ...
    if (this.filteredCalls.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" class="loading">No calls found for this period</td></tr>';
      return;
    }

    // Sort calls by start time (most recent first)
    const sortedCalls = [...this.filteredCalls].sort(
    ...
    sortedCalls.forEach((call) => {
      ...
      html += `
                <tr>
                    ...
                    <td>${
                      call.serviced ? (call.earnings || 0).toFixed(2) : "NS"
                    }</td>
                    <td class="hidden deleteRow">
  ...
  updateHeaderStats() {
    ...
    const todaysCalls = this.allCalls.filter(
    ...
    const monthCalls = this.allCalls.filter((call) => {
      const callDate = new Date(call.startTime);
      return (
        callDate.getMonth() === thisMonth && callDate.getFullYear() === thisYear
      );
    });

    const todayEarnings = todaysCalls.reduce(
      (sum, call) => sum + (call.earnings || 0),
      0
    );
    const monthEarnings = monthCalls.reduce(
      (sum, call) => sum + (call.earnings || 0),
      0
    );
    const totalEarnings = this.allCalls.reduce(
      (sum, call) => sum + (call.earnings || 0),
      0
    );
    ...
    document.getElementById(
      "todayCalls"
    ).textContent = `${todaysCalls.length} calls`;

    document.getElementById(
      "monthEarnings"
    ).textContent = `${monthEarnings.toFixed(2)}`;
    document.getElementById(
      "monthCalls"
    ).textContent = `${monthCalls.length} calls`;
    ...
    document.getElementById(
      "totalCalls"
    ).textContent = `${this.allCalls.length} calls`;
  }
    ...
    for (let i = 0; i < 42; i++) {
      ...
      html += `
                <div class="${dayClass}" title="${earnings.toFixed(2)}">
                    ...
                    ${
                      earnings > 0
                        ? `<div class="day-earnings">${earnings.toFixed(
                            2
                          )}</div>`
                        : ""
                    }
  ...
  exportToCSV() {
    ...
    a.href = url;
    a.download = `call-tracker-export-${
  ...
  async clearAllData() {
    if (
      confirm(
        "Are you sure you want to clear all call data? This cannot be undone."
      )
    ) {
      try {
  ...
  editCalls() {
    const col = document.getElementById("deleteCol");
  ...
  async deleteCall(callID) {
    try {
      const result = await chrome.storage.local.get(["calls"]);
      const calls = result.calls || {};
      delete calls[callID]; // ✅ Removes the key from the object
