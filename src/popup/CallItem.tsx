...
export const CallItem = ({ call, roundCorners }: CallItemProps) => {
  ...
  return (
    <>
      <div className={`bg-accent p-2.5 flex justify-between items-center ${roundCornersClass()} border-b`}>
        <div>
          <p className="text-base">{dateUtils.formatTime(call.startTime)} - {call.endTime? dateUtils.formatTime(call.endTime) : ""}</p>
          {call.duration? <p className="text-muted-foreground text-sm">{dateUtils.formatDuration(call.duration)}</p> : null}
        </div>
      ...
      {call.available? 
        <div className="bg-accent-foreground/5 dark:bg-accent-foreground/5 p-1 text-center border-b ">
          <p className="text-muted-foreground text-xs">{dateUtils.formatDuration(call.available)}</p>
        </div>
        :
        null
      }
